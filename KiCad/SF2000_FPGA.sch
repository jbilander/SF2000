EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 2 5
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L SF2000:10M04SCE144 U7
U 1 1 62146C83
P 3800 3550
AR Path="/62146C83" Ref="U7"  Part="1" 
AR Path="/620D263D/62146C83" Ref="U8"  Part="1" 
F 0 "U8" H 3000 3550 60  0000 R CNN
F 1 "10M04SCE144" H 3250 3400 60  0000 R CNN
F 2 "SF2000:EQFP-144-1EP_20x20mm_P0.5mm_EP5x5mm_TH_GND_pad" H 3800 3900 60  0001 C CNN
F 3 "https://www.mouser.se/datasheet/2/612/m10_overview-2401081.pdf" H 5250 4750 60  0001 C CNN
	1    3800 3550
	1    0    0    -1  
$EndComp
Text GLabel 3800 2900 1    50   Input ~ 0
GND
Text GLabel 3800 4500 3    50   Input ~ 0
GND
Wire Wire Line
	3200 4500 3300 4500
Connection ~ 3300 4500
Wire Wire Line
	3300 4500 3400 4500
Connection ~ 3400 4500
Wire Wire Line
	3400 4500 3500 4500
Connection ~ 3500 4500
Wire Wire Line
	3500 4500 3600 4500
Connection ~ 3600 4500
Wire Wire Line
	3600 4500 3700 4500
Connection ~ 3700 4500
Wire Wire Line
	3700 4500 3800 4500
Connection ~ 3800 4500
Wire Wire Line
	3800 4500 3900 4500
Connection ~ 3900 4500
Wire Wire Line
	3900 4500 4000 4500
Connection ~ 4000 4500
Wire Wire Line
	4000 4500 4100 4500
Connection ~ 4100 4500
Wire Wire Line
	4100 4500 4200 4500
Connection ~ 4200 4500
Wire Wire Line
	4200 4500 4300 4500
Connection ~ 4300 4500
Wire Wire Line
	4300 4500 4400 4500
Connection ~ 4400 4500
Wire Wire Line
	4400 4500 4500 4500
Text GLabel 1350 4850 0    50   Output ~ 0
CLKCPU_3V3
Text GLabel 1350 4350 0    50   Input ~ 0
C7M_3V3
Text GLabel 1350 4250 0    50   Input ~ 0
CDAC_3V3
Text GLabel 1350 2500 0    50   Output ~ 0
ROM_WE
Text GLabel 1350 2600 0    50   Output ~ 0
OVR_3V3
Text GLabel 1350 2700 0    50   Output ~ 0
INT2_3V3
Wire Wire Line
	2100 1100 2200 1100
Connection ~ 2200 1100
Wire Wire Line
	2200 1100 2300 1100
Connection ~ 2300 1100
Wire Wire Line
	2300 1100 2400 1100
Connection ~ 2400 1100
Wire Wire Line
	2400 1100 2500 1100
Connection ~ 2500 1100
Wire Wire Line
	2500 1100 2600 1100
Connection ~ 2600 1100
Wire Wire Line
	2600 1100 2700 1100
Connection ~ 2700 1100
Wire Wire Line
	2700 1100 2800 1100
Connection ~ 2800 1100
Wire Wire Line
	2800 1100 2900 1100
Connection ~ 2900 1100
Wire Wire Line
	2900 1100 3000 1100
Connection ~ 3000 1100
Wire Wire Line
	3000 1100 3100 1100
Connection ~ 3100 1100
Wire Wire Line
	3100 1100 3200 1100
Connection ~ 3200 1100
Wire Wire Line
	3200 1100 3400 1100
Connection ~ 3400 1100
Wire Wire Line
	3400 1100 3500 1100
Connection ~ 3500 1100
Wire Wire Line
	3500 1100 3600 1100
Connection ~ 3600 1100
Wire Wire Line
	3600 1100 3700 1100
Connection ~ 3700 1100
Wire Wire Line
	3700 1100 3800 1100
Connection ~ 3800 1100
Wire Wire Line
	3800 1100 3900 1100
Connection ~ 3900 1100
Wire Wire Line
	3900 1100 4000 1100
Connection ~ 4000 1100
Wire Wire Line
	4000 1100 4200 1100
Connection ~ 4200 1100
Wire Wire Line
	4200 1100 4300 1100
Connection ~ 4300 1100
Wire Wire Line
	4300 1100 4400 1100
Connection ~ 4400 1100
Wire Wire Line
	4400 1100 4500 1100
Connection ~ 4500 1100
Wire Wire Line
	4500 1100 4600 1100
Connection ~ 4600 1100
Wire Wire Line
	4600 1100 4700 1100
Connection ~ 4700 1100
Wire Wire Line
	4700 1100 4800 1100
Connection ~ 4800 1100
Wire Wire Line
	4800 1100 4900 1100
Connection ~ 4900 1100
Wire Wire Line
	4900 1100 5000 1100
Connection ~ 5000 1100
Wire Wire Line
	5000 1100 5100 1100
Text GLabel 3300 1100 1    50   Input ~ 0
3V3
Text GLabel 1350 2800 0    50   Output ~ 0
BOSS_3V3
Text GLabel 1350 2900 0    50   Output ~ 0
INT6_3V3
Text GLabel 1350 4450 0    50   Input ~ 0
FC0_3V3
Text GLabel 1350 4550 0    50   Input ~ 0
FC1_3V3
Text GLabel 1350 4750 0    50   Input ~ 0
FC2_3V3
Text GLabel 6250 2200 2    50   Input ~ 0
A20_3V3
Text GLabel 6250 2000 2    50   Input ~ 0
A22_3V3
Text GLabel 6250 2100 2    50   Input ~ 0
A21_3V3
Text GLabel 6250 3100 2    50   Input ~ 0
A12_3V3
Text GLabel 3850 6000 3    50   Input ~ 0
UDS_3V3
Text GLabel 3750 6000 3    50   Input ~ 0
LDS_3V3
Text GLabel 2300 6000 3    50   Input ~ 0
RW_3V3
Text GLabel 6250 2900 2    50   Input ~ 0
A13_3V3
Text GLabel 6250 2500 2    50   Input ~ 0
A18_3V3
Text GLabel 6250 2300 2    50   Input ~ 0
A19_3V3
Text GLabel 6250 2800 2    50   Input ~ 0
A16_3V3
Text GLabel 6250 2700 2    50   Input ~ 0
A17_3V3
Text GLabel 1350 2200 0    50   Input ~ 0
VPA_3V3
Text GLabel 1350 3600 0    50   BiDi ~ 0
E_3V3
Text GLabel 1350 3100 0    50   Input ~ 0
TMS
Text GLabel 1350 4650 0    50   Input ~ 0
AS_CPU_3V3
Text GLabel 6250 1500 2    50   Input ~ 0
CFGIN_3V3
Text GLabel 6250 1600 2    50   Output ~ 0
CFGOUT_3V3
Text GLabel 6250 3000 2    50   Input ~ 0
RST_3V3
Text GLabel 1900 6000 3    50   Input ~ 0
HLT_3V3
Text GLabel 2000 6000 3    50   Input ~ 0
BR_3V3
Text GLabel 2100 6000 3    50   Input ~ 0
BG_3V3
Text GLabel 2200 6000 3    50   Output ~ 0
DTACK_CPU_3V3
Text GLabel 2400 6000 3    50   Input ~ 0
DTACK_MB_3V3
Text GLabel 2500 6000 3    50   Input ~ 0
BGACK_3V3
Text GLabel 2600 6000 3    50   Output ~ 0
AS_MB_3V3
$Comp
L Connector_Generic:Conn_02x05_Odd_Even JTAG1
U 1 1 625BCAC6
P 8200 3400
F 0 "JTAG1" H 8250 2975 50  0000 C CNN
F 1 "Conn_02x05_Odd_Even" H 8250 3066 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_2x05_P2.54mm_Vertical" H 8200 3400 50  0001 C CNN
F 3 "~" H 8200 3400 50  0001 C CNN
	1    8200 3400
	-1   0    0    1   
$EndComp
Text GLabel 7900 3200 0    50   Input ~ 0
GND
Text GLabel 7900 3600 0    50   Input ~ 0
GND
Text GLabel 7900 3500 0    50   Input ~ 0
3V3
Text GLabel 8400 3600 2    50   Input ~ 0
TCK
Text GLabel 8400 3500 2    50   Input ~ 0
TDO
Text GLabel 8400 3200 2    50   Input ~ 0
TDI
Text GLabel 8400 3400 2    50   Input ~ 0
TMS
NoConn ~ 7900 3400
NoConn ~ 8400 3300
Text GLabel 1350 3300 0    50   Input ~ 0
TCK
Text GLabel 1350 3500 0    50   Output ~ 0
TDO
Text GLabel 1350 3400 0    50   Input ~ 0
TDI
Text GLabel 6250 1800 2    50   Input ~ 0
nSTATUS
Text GLabel 6250 1700 2    50   Input ~ 0
CONF_DONE
Text GLabel 6250 2600 2    50   Input ~ 0
CONFIG_SEL
Text GLabel 1350 2400 0    50   Input ~ 0
SW1
Text GLabel 7900 4850 0    50   Input ~ 0
JP5
$Comp
L Device:R_Pack04 RN1
U 1 1 625F56E7
P 9300 4250
F 0 "RN1" H 9488 4296 50  0000 L CNN
F 1 "10k_Pack04" H 9488 4205 50  0000 L CNN
F 2 "SF2000:RESCAF80P320X160X60-8N" V 9575 4250 50  0001 C CNN
F 3 "~" H 9300 4250 50  0001 C CNN
	1    9300 4250
	1    0    0    -1  
$EndComp
Text GLabel 9100 4050 1    50   Input ~ 0
3V3
Text GLabel 9100 4450 3    50   Input ~ 0
TDI
Text GLabel 9200 4450 3    50   Input ~ 0
TCK
Text GLabel 9400 4450 3    50   Input ~ 0
TMS
Text GLabel 9300 4450 3    50   Input ~ 0
JP5
Text GLabel 9200 4050 1    50   Input ~ 0
GND
Text GLabel 1350 3700 0    50   Input ~ 0
JP2
Text GLabel 9300 4050 1    50   Input ~ 0
3V3
Wire Wire Line
	9300 4050 9400 4050
$Comp
L Device:R_Pack04 RN2
U 1 1 6265DCDE
P 10400 4250
F 0 "RN2" H 10588 4296 50  0000 L CNN
F 1 "10k_Pack04" H 10588 4205 50  0000 L CNN
F 2 "SF2000:RESCAF80P320X160X60-8N" V 10675 4250 50  0001 C CNN
F 3 "~" H 10400 4250 50  0001 C CNN
	1    10400 4250
	1    0    0    -1  
$EndComp
Text GLabel 10200 4050 1    50   Input ~ 0
3V3
Wire Wire Line
	10200 4050 10300 4050
Connection ~ 10300 4050
Wire Wire Line
	10300 4050 10400 4050
Connection ~ 10400 4050
Wire Wire Line
	10400 4050 10500 4050
Text GLabel 10200 5400 3    50   Input ~ 0
nSTATUS
Text GLabel 10300 5400 3    50   Input ~ 0
CONF_DONE
Text GLabel 10200 4450 3    50   Input ~ 0
JP4
Text GLabel 10300 4450 3    50   Input ~ 0
JP3
Text GLabel 7500 1650 1    50   Input ~ 0
3V3
$Comp
L Connector_Generic:Conn_02x03_Odd_Even J3
U 1 1 62704BD7
P 8200 4250
F 0 "J3" H 8250 3900 50  0000 C CNN
F 1 "Conn_02x03_Odd_Even" H 8250 4000 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x03_P2.00mm_Vertical" H 8200 4250 50  0001 C CNN
F 3 "~" H 8200 4250 50  0001 C CNN
	1    8200 4250
	-1   0    0    1   
$EndComp
Text GLabel 7900 4250 0    50   Input ~ 0
JP3
Text GLabel 7900 4350 0    50   Input ~ 0
JP2
Text GLabel 7900 4150 0    50   Input ~ 0
JP4
Wire Wire Line
	8400 4150 8400 4250
Connection ~ 8400 4250
Wire Wire Line
	8400 4250 8400 4350
Text GLabel 8400 4150 2    50   Input ~ 0
GND
Text GLabel 1350 3800 0    50   Input ~ 0
JP3
Text GLabel 1350 3900 0    50   Input ~ 0
JP4
$Comp
L Connector_Generic:Conn_02x01 J4
U 1 1 6276C760
P 8200 4850
F 0 "J4" H 8250 4600 50  0000 C CNN
F 1 "Conn_02x01" H 8250 4700 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x01_P2.00mm_Vertical" H 8200 4850 50  0001 C CNN
F 3 "~" H 8200 4850 50  0001 C CNN
	1    8200 4850
	-1   0    0    1   
$EndComp
Text GLabel 8400 4850 2    50   Input ~ 0
GND
Text GLabel 9700 2050 2    50   Output ~ 0
SW1
Text GLabel 1350 3200 0    50   Input ~ 0
JP5
Text GLabel 10400 4450 3    50   Input ~ 0
JP2
Text GLabel 1350 2300 0    50   Output ~ 0
VMA_3V3
Text GLabel 2800 6000 3    50   BiDi ~ 0
D9_3V3
Text GLabel 2900 6000 3    50   BiDi ~ 0
D10_3V3
Text GLabel 3000 6000 3    50   BiDi ~ 0
D11_3V3
Text GLabel 3100 6000 3    50   BiDi ~ 0
D12_3V3
Text GLabel 3200 6000 3    50   BiDi ~ 0
D13_3V3
Text GLabel 3300 6000 3    50   BiDi ~ 0
D14_3V3
Text GLabel 3400 6000 3    50   BiDi ~ 0
D15_3V3
Text GLabel 2700 6000 3    50   BiDi ~ 0
D8_3V3
Text GLabel 4150 6000 3    50   Output ~ 0
FLASH_WE
Text GLabel 4050 6000 3    50   Output ~ 0
FLASH_RST
Text GLabel 3950 6000 3    50   Input ~ 0
FLASH_BUSY
Text GLabel 5400 6000 3    50   BiDi ~ 0
SD_D0
Text GLabel 5500 6000 3    50   BiDi ~ 0
SD_D1
Text GLabel 4600 6000 3    50   BiDi ~ 0
SD_D2
Text GLabel 4700 6000 3    50   BiDi ~ 0
SD_D3
Text GLabel 4800 6000 3    50   Output ~ 0
SD_CMD
Text GLabel 5000 6000 3    50   Output ~ 0
SD_CLK
Text GLabel 5600 6000 3    50   Input ~ 0
SD_CD
Text GLabel 6250 4000 2    50   BiDi ~ 0
D0_3V3
Text GLabel 6250 4100 2    50   BiDi ~ 0
D1_3V3
Text GLabel 6250 4300 2    50   BiDi ~ 0
D2_3V3
Text GLabel 6250 4600 2    50   BiDi ~ 0
D3_3V3
Text GLabel 6250 4800 2    50   BiDi ~ 0
D4_3V3
Text GLabel 6250 5000 2    50   BiDi ~ 0
D5_3V3
Text GLabel 6250 5100 2    50   BiDi ~ 0
D6_3V3
Text GLabel 6250 5200 2    50   BiDi ~ 0
D7_3V3
Text GLabel 6250 4900 2    50   Input ~ 0
A5_3V3
Text GLabel 6250 3250 2    50   Input ~ 0
A6_3V3
Text GLabel 6250 4700 2    50   Input ~ 0
A4_3V3
Text GLabel 6250 4500 2    50   Input ~ 0
A3_3V3
Text GLabel 6250 1900 2    50   Input ~ 0
A23_3V3
Text GLabel 6250 4400 2    50   Input ~ 0
A2_3V3
Text GLabel 6250 4200 2    50   Input ~ 0
A1_3V3
Text GLabel 5300 6000 3    50   Input ~ 0
IDE_IOW_3V3
Text GLabel 5200 6000 3    50   Input ~ 0
IDE_IOR_3V3
Text GLabel 5100 6000 3    50   Input ~ 0
IDE_CS1_3V3
Text GLabel 4900 6000 3    50   Input ~ 0
IDE_CS0_3V3
Text GLabel 5700 6000 3    50   Output ~ 0
FLASH_A19
Text GLabel 6250 5400 2    50   Input ~ 0
OSC_CLK
Text GLabel 6250 3750 2    50   Input ~ 0
OE_BANK1
Text GLabel 6250 3350 2    50   Input ~ 0
WE_BANK1_EVEN
Text GLabel 6250 3850 2    50   Input ~ 0
WE_BANK1_ODD
Text GLabel 6250 3450 2    50   Input ~ 0
OE_BANK0
Text GLabel 6250 3550 2    50   Input ~ 0
WE_BANK0_EVEN
Text GLabel 6250 3650 2    50   Input ~ 0
WE_BANK0_ODD
$Comp
L Device:R_Pack04 RN6
U 1 1 62107318
P 10400 5200
F 0 "RN6" H 10588 5246 50  0000 L CNN
F 1 "10k_Pack04" H 10588 5155 50  0000 L CNN
F 2 "SF2000:RESCAF80P320X160X60-8N" V 10675 5200 50  0001 C CNN
F 3 "~" H 10400 5200 50  0001 C CNN
	1    10400 5200
	1    0    0    -1  
$EndComp
Text GLabel 10500 5400 3    50   Input ~ 0
CONFIG_SEL
Text GLabel 10200 5000 1    50   Input ~ 0
3V3
Wire Wire Line
	10200 5000 10300 5000
Text GLabel 10500 5000 1    50   Input ~ 0
GND
Text GLabel 6250 2400 2    50   Input ~ 0
JP7
$Comp
L Connector_Generic:Conn_02x01 J9
U 1 1 62127717
P 8200 5650
F 0 "J9" H 8250 5400 50  0000 C CNN
F 1 "Conn_02x01" H 8250 5500 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x01_P2.00mm_Vertical" H 8200 5650 50  0001 C CNN
F 3 "~" H 8200 5650 50  0001 C CNN
	1    8200 5650
	-1   0    0    1   
$EndComp
Text GLabel 8400 5650 2    50   Input ~ 0
GND
Text GLabel 7900 5650 0    50   Input ~ 0
JP7
$Comp
L Connector_Generic:Conn_02x01 J10
U 1 1 62132436
P 8200 6050
F 0 "J10" H 8250 5800 50  0000 C CNN
F 1 "Conn_02x01" H 8250 5900 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x01_P2.00mm_Vertical" H 8200 6050 50  0001 C CNN
F 3 "~" H 8200 6050 50  0001 C CNN
	1    8200 6050
	-1   0    0    1   
$EndComp
Text GLabel 8400 6050 2    50   Input ~ 0
GND
Text GLabel 7900 6050 0    50   Input ~ 0
JP8
Text GLabel 6250 5300 2    50   Input ~ 0
JP8
Text GLabel 3500 6000 3    50   Output ~ 0
FLASH_OE
Text GLabel 1350 3000 0    50   Output ~ 0
ROM_OE
Text GLabel 4250 6000 3    50   Output ~ 0
ROM_B1
Text GLabel 4350 6000 3    50   Output ~ 0
ROM_B2
Wire Wire Line
	10300 5000 10400 5000
Connection ~ 10300 5000
Text GLabel 10400 5400 3    50   Input ~ 0
JP7
Text GLabel 7900 5250 0    50   Input ~ 0
JP6
$Comp
L Connector_Generic:Conn_02x01 J8
U 1 1 621772F2
P 8200 5250
F 0 "J8" H 8250 5000 50  0000 C CNN
F 1 "Conn_02x01" H 8250 5100 50  0000 C CNN
F 2 "Connector_PinHeader_2.00mm:PinHeader_2x01_P2.00mm_Vertical" H 8200 5250 50  0001 C CNN
F 3 "~" H 8200 5250 50  0001 C CNN
	1    8200 5250
	-1   0    0    1   
$EndComp
Text GLabel 8400 5250 2    50   Input ~ 0
GND
Text GLabel 10500 4450 3    50   Input ~ 0
JP6
Text GLabel 1800 6000 3    50   Input ~ 0
JP6
$Comp
L SF2000:74LVC2G14 U3
U 1 1 623FD973
P 9250 2250
F 0 "U3" H 9250 2765 50  0000 C CNN
F 1 "74LVC2G14" H 9250 2674 50  0000 C CNN
F 2 "Package_SO:TSOP-6_1.65x3.05mm_P0.95mm" H 9750 3200 50  0001 C CNN
F 3 "https://assets.nexperia.com/documents/data-sheet/74LVC2G14.pdf" H 9750 3200 50  0001 C CNN
	1    9250 2250
	1    0    0    -1  
$EndComp
Text GLabel 8450 2350 3    50   Input ~ 0
GND
Text GLabel 10150 2250 2    50   Input ~ 0
3V3
Wire Wire Line
	8450 2050 8800 2050
$Comp
L Device:C C?
U 1 1 624A6DA8
P 8450 2200
AR Path="/624A6DA8" Ref="C?"  Part="1" 
AR Path="/620D263D/624A6DA8" Ref="C46"  Part="1" 
F 0 "C46" H 8500 2300 50  0000 L CNN
F 1 "0.1uF" H 8450 2100 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 8488 2050 50  0001 C CNN
F 3 "~" H 8450 2200 50  0001 C CNN
	1    8450 2200
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 624B4CFC
P 9900 2400
AR Path="/624B4CFC" Ref="C?"  Part="1" 
AR Path="/620D263D/624B4CFC" Ref="C47"  Part="1" 
F 0 "C47" H 9950 2500 50  0000 L CNN
F 1 "0.1uF" H 9900 2300 50  0000 L CNN
F 2 "Capacitor_SMD:C_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 9938 2250 50  0001 C CNN
F 3 "~" H 9900 2400 50  0001 C CNN
	1    9900 2400
	1    0    0    -1  
$EndComp
Text GLabel 9900 2550 3    50   Input ~ 0
GND
Wire Wire Line
	9700 2250 9900 2250
Connection ~ 9900 2250
Wire Wire Line
	9900 2250 10150 2250
$Comp
L Device:R_Small R?
U 1 1 62517BB4
P 7500 1850
AR Path="/62517BB4" Ref="R?"  Part="1" 
AR Path="/620D263D/62517BB4" Ref="R4"  Part="1" 
F 0 "R4" H 7559 1896 50  0000 L CNN
F 1 "10k" H 7559 1805 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 7500 1850 50  0001 C CNN
F 3 "~" H 7500 1850 50  0001 C CNN
	1    7500 1850
	1    0    0    -1  
$EndComp
$Comp
L Device:R_Small R?
U 1 1 62519236
P 8000 2050
AR Path="/62519236" Ref="R?"  Part="1" 
AR Path="/620D263D/62519236" Ref="R7"  Part="1" 
F 0 "R7" V 8100 2000 50  0000 L CNN
F 1 "10k" V 7900 2000 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 8000 2050 50  0001 C CNN
F 3 "~" H 8000 2050 50  0001 C CNN
	1    8000 2050
	0    1    1    0   
$EndComp
Wire Wire Line
	7500 1750 7500 1650
Wire Wire Line
	7900 3300 7500 3300
Wire Wire Line
	7500 1950 7500 2050
Wire Wire Line
	7900 2050 7500 2050
Connection ~ 7500 2050
Wire Wire Line
	7500 2050 7500 3300
Wire Wire Line
	8100 2050 8450 2050
Connection ~ 8450 2050
Wire Wire Line
	8450 2350 8700 2350
Wire Wire Line
	8700 2350 8700 2250
Wire Wire Line
	8700 2250 8800 2250
Text GLabel 8650 2700 0    50   Input ~ 0
ACTIVE
Wire Wire Line
	8800 2450 8700 2450
Wire Wire Line
	8700 2450 8700 2700
Wire Wire Line
	8700 2700 8650 2700
$Comp
L Device:R_Small R?
U 1 1 625C5BBC
P 9900 3050
AR Path="/625C5BBC" Ref="R?"  Part="1" 
AR Path="/620D263D/625C5BBC" Ref="R8"  Part="1" 
F 0 "R8" V 10000 3000 50  0000 L CNN
F 1 "220" V 9800 3000 50  0000 L CNN
F 2 "Resistor_SMD:R_0805_2012Metric_Pad1.15x1.40mm_HandSolder" H 9900 3050 50  0001 C CNN
F 3 "~" H 9900 3050 50  0001 C CNN
	1    9900 3050
	0    1    1    0   
$EndComp
Wire Wire Line
	9700 2450 9700 3050
Wire Wire Line
	9700 3050 9800 3050
$Comp
L Connector_Generic:Conn_02x01 J11
U 1 1 625CA23F
P 10450 3050
F 0 "J11" H 10500 2800 50  0000 C CNN
F 1 "Conn_02x01" H 10500 2900 50  0000 C CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_2x01_P2.54mm_Vertical" H 10450 3050 50  0001 C CNN
F 3 "~" H 10450 3050 50  0001 C CNN
	1    10450 3050
	-1   0    0    1   
$EndComp
Wire Wire Line
	10000 3050 10150 3050
Text GLabel 10650 3050 2    50   Input ~ 0
GND
Text Label 10150 3050 3    50   ~ 0
IDE_LED
$Comp
L Connector_Generic:Conn_01x01 J12
U 1 1 62602244
P 9000 2900
F 0 "J12" H 9080 2942 50  0000 L CNN
F 1 "Conn_01x01" H 9080 2851 50  0000 L CNN
F 2 "Connector_PinHeader_2.54mm:PinHeader_1x01_P2.54mm_Vertical" H 9000 2900 50  0001 C CNN
F 3 "~" H 9000 2900 50  0001 C CNN
	1    9000 2900
	1    0    0    -1  
$EndComp
Wire Wire Line
	8700 2700 8700 2900
Wire Wire Line
	8700 2900 8800 2900
Connection ~ 8700 2700
$EndSCHEMATC
